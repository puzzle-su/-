...
def get_market_data():
    ...
    return results, sp500_rsi, sp500_rsi_div, gold_price, silver_price
...
# ==========================================
def get_extreme_signals(sp500_rsi, s5fi_val, pcr_5ma, s5fi_div=None, sp500_rsi_div=None):
    ...
    if sp500_rsi is not None:
        ...
        rsi_msg = f"3. 標普大盤 RSI (14日) : {rsi_val} / 門檻小於30或大於70 [{status}]"
        
        if sp500_rsi_div and sp500_rsi_div.strip() != "":
            rsi_msg += f"\n   ➤ {sp500_rsi_div}"
            if "買入" in sp500_rsi_div or "買訊" in sp500_rsi_div or "底背離" in sp500_rsi_div:
                buy_count += 1
            elif "賣出" in sp500_rsi_div or "賣訊" in sp500_rsi_div or "頂背離" in sp500_rsi_div:
                sell_count += 1
                
        signals.append(rsi_msg)
    else:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--emergency':
        ...
        try:
            ...
            if len(hist) >= 60:
                ...
                if recent_px.max() > past_px.max():
                    if recent_rsi.loc[recent_high_idx] < past_rsi.loc[past_high_idx] - 5:
                        if hist['Close'].iloc[-1] >= recent_px.max() * 0.98:
                            sp500_rsi_div = " ⚠️ RSI頂背離"
                            
                past_low_idx = past_px.idxmin()
                recent_low_idx = recent_px.idxmin()
                if recent_px.min() < past_px.min():
                    if recent_rsi.loc[recent_low_idx] > past_rsi.loc[past_low_idx] + 5:
                        if hist['Close'].iloc[-1] <= recent_px.min() * 1.02:
                            sp500_rsi_div = " 🔔 RSI底背離"
        except:
            sp500_rsi = None
            sp500_rsi_div = None
        ...
        pcr_5ma = get_pcr_5ma()
        extreme_signals, buy_count, sell_count = get_extreme_signals(sp500_rsi, s5fi_val, pcr_5ma, s5fi_div, sp500_rsi_div)
    ...
    market_data, sp500_rsi, sp500_rsi_div, gold_price, silver_price = get_market_data()
